...
void VulkanAPITestManager::Release()
{
    if (m_red_error_img_mem != VK_NULL_HANDLE) {
        vkFreeMemory(m_VK_device, m_red_error_img_mem, nullptr);
        m_red_error_img_mem = VK_NULL_HANDLE;
    }

    if (m_red_error_img != VK_NULL_HANDLE) {
        vkDestroyImage(m_VK_device, m_red_error_img, nullptr);
        m_red_error_img = VK_NULL_HANDLE;
    }
    //
    std::vector<SampleStrongReferenceObject>::iterator iter;
...
void VulkanAPITestManager::RenderToScreen()
{
    ...
    if (cb_iv != VK_NULL_HANDLE) {
        ...
        SetMainViewportDynamically(m_VK_main_cmd_buffer, viewport);
        SetMainScissorDynamically(m_VK_main_cmd_buffer, render_area);

        BindGraphicsPipeline(m_VK_main_cmd_buffer, m_VK_screen_pipeline);

        std::vector<VkDescriptorSet> descs = { m_VK_screen_desc_set };
        std::vector<uint32_t> dynamic_offs = {};
        BindDescriptorSets(m_VK_main_cmd_buffer, m_VK_screen_pipeline_layout, VK_PIPELINE_BIND_POINT_GRAPHICS, 0, descs, dynamic_offs);

        //Bind Vertex Buffer.
        BindVertexBuffer(m_VK_main_cmd_buffer, m_VK_screen_ver_buffer, 0, 0); //vertex
        BindVertexBuffer(m_VK_main_cmd_buffer, m_VK_screen_ver_tex_buffer, 0, 1); //tex
        BindIndiceBuffer(m_VK_main_cmd_buffer, m_VK_screen_indices_buffer, 0, VK_INDEX_TYPE_UINT16); //Bind indices.
        DrawByIndice(m_VK_main_cmd_buffer, 6, 1, 0, 0, 0);
    }
...
{
    vkUpdateDescriptorSets(m_VK_device, static_cast<uint32_t>(i_descriptor_w_infos.size()), i_descriptor_w_infos.data(), 0, VK_NULL_HANDLE);
}
