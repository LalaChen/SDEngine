...
void Sample1_DrawTriangle::CreateShaderPrograms()
{
    ...
    var_main_texture.pImmutableSamplers = nullptr;
    ...
    if (result != VK_SUCCESS) {
        SDLOGE("Create descriptor set layout failure!!!");
        return;
    }
    else {
        SDLOG("Create descriptor set layout successfully.");
    }
    ...
    if (result != VK_SUCCESS) {
        ...
        SDLOG("Create descriptor pool successfully.");
    }
    ...
    if (result != VK_SUCCESS) {
        ...
        SDLOG("Allocate descriptor set 0 successfully.");
    }
    //Object 1.
    ...
    if (result != VK_SUCCESS) {
        ...
        SDLOG("Allocate descriptor set 1 successfully.");
    }
    ...
    for (uint32_t i = 0; i < 2; ++i) {
        ...
        SDLOG("UpdateDescriptorSet for object(%d)", i);
    }
    //3. build shader.
    //--- i. read shader file.
    ...
    if (result != VK_SUCCESS) {
        ...
        SDLOG("Create vert shader successfully.");
    }
    ...
    if (result != VK_SUCCESS) {
        ...
        SDLOG("Create frag shader successfully.");
    }
    ...
    if (result != VK_SUCCESS) {
        ...
        SDLOG("create pipeline layout successfully.");
    }
    ...
    result = m_mgr->CreateGraphicsPipeline(graphics_pipeline_c_info, SD_NULL_HANDLE, m_VK_main_graphics_pipeline);
    if (result != VK_SUCCESS) {
        SDLOGE("create pipeline failure!!!");
        return;
    }
    else {
        SDLOG("create pipeline successfully.");
    }
}
...
void Sample1_DrawTriangle::CreateRenderPass()
{
    ...
    sp_dependencies[1].srcStageMask = VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT;
