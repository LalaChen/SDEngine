...
void CommandRecordingThread::Initialize(uint32_t i_default_cbs)
{
    ...
    for (uint32_t cbID = 0; cbID < i_default_cbs; ++cbID) {
        m_cbs.push_back(m_cp.GetRef().AllocateCommandBuffer(StringFormat("cb_%d", cbID), CommandBufferLevel_SECONDARY));
    }
    m_current_cb_iter = m_cbs.begin();
...
void CommandRecordingThread::Run()
{
    while (m_stop == false) {
        ...
        if (m_current_cb_iter == m_cbs.end()) {
            size_t cb_size = m_cbs.size();
            m_cbs.push_back(m_cp.GetRef().AllocateCommandBuffer(StringFormat("cb_%d", cb_size), CommandBufferLevel_SECONDARY));
            m_current_cb_iter = m_cbs.end();
            m_current_cb_iter--;
        }
...
void CommandRecordingThread::Wait(std::list<CommandBufferWeakReferenceObject> &io_submitted_list)
{
    ...
    for (std::list<CommandBufferWeakReferenceObject>::iterator cb_iter = m_cbs.begin(); cb_iter != m_current_cb_iter; ++cb_iter) {
