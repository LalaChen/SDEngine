...
//for camera
, m_VK_color_buffers{VK_NULL_HANDLE}
, m_VK_color_buffer_memories{VK_NULL_HANDLE}
, m_VK_depth_buffers{VK_NULL_HANDLE}
, m_VK_depth_buffer_memories{VK_NULL_HANDLE}
//for camera
...
void Sample3_MultiSubpass::Render()
{
    if (m_mgr != nullptr) {
        ...
        //--- world space. 
        m_uniform_buffer_datas[2].m_worid.translate(Vector3f(0.3f, 0.3f, 0.09f, 1.0f));
        ...
        //--- world space. 
        m_uniform_buffer_datas[3].m_worid.translate(Vector3f(-0.3f, -0.3f, 0.08f, 1.0f));
...
void Sample3_MultiSubpass::Destroy()
{
    if (m_mgr != nullptr) {
        ...
        for (uint32_t i = 0; i < 2; ++i) {
            m_mgr->DestroyImage(m_VK_color_buffers[i]);
            m_VK_color_buffers[i] = VK_NULL_HANDLE;
            m_mgr->ReleaseMemory(m_VK_color_buffer_memories[i]);
            m_VK_color_buffer_memories[i] = VK_NULL_HANDLE;
            m_mgr->DestroyImageView(m_VK_color_buffer_image_views[i]);
            m_VK_color_buffer_image_views[i] = VK_NULL_HANDLE;

            m_mgr->DestroyImage(m_VK_depth_buffers[i]);
            m_VK_depth_buffers[i] = VK_NULL_HANDLE;
            m_mgr->ReleaseMemory(m_VK_depth_buffer_memories[i]);
            m_VK_depth_buffer_memories[i] = VK_NULL_HANDLE;
...
void Sample3_MultiSubpass::CreateShaderPrograms()
{
    ...
    for (uint32_t i = 0; i < COLOR_AND_DEPTH_BUFFER_SIZE; ++i) {
        ...
        if (FileSystemManager::GetRef().OpenFile(vert_fns[i], vert_shader) == false) {
            SDLOGE("%s isn't exist!!!", vert_fns[i]);
            return;
        }

        if (FileSystemManager::GetRef().OpenFile(frag_fns[i], frag_shader) == false) {
        ...
        result = m_mgr->CreateGraphicsPipeline(graphics_pipeline_c_info, nullptr, m_VK_main_graphics_pipelines[i]);
        if (result != VK_SUCCESS) {
...
void Sample3_MultiSubpass::CreateRenderPassAndFramebuffer()
{
    ...
    for (uint32_t i = 0; i < COLOR_AND_DEPTH_BUFFER_SIZE; ++i) {
        result = m_mgr->CreateImage(cb_c_info, m_VK_color_buffers[i]);
        if (result != VK_SUCCESS) {
            SDLOGE("Sample1 color buffer create failure.");
            return;
        }

        result = m_mgr->AllocateMemoryAndBindToImage(m_VK_color_buffers[i], VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 0, m_VK_color_buffer_memories[i]);
        if (result != VK_SUCCESS) {
    ...
    for (uint32_t i = 0; i < COLOR_AND_DEPTH_BUFFER_SIZE; ++i) {
        result = m_mgr->CreateImage(db_c_info, m_VK_depth_buffers[i]);
        if (result != VK_SUCCESS) {
            SDLOGE("Sample1 depth buffer create failure.");
            return;
        }

        result = m_mgr->AllocateMemoryAndBindToImage(m_VK_depth_buffers[i], VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 0, m_VK_depth_buffer_memories[i]);
        if (result != VK_SUCCESS) {
    ...
    for (uint32_t i = 0; i < 1; ++i) {
        ...
        img_view_cb_info.image = m_VK_color_buffers[i];
        ...
        result = m_mgr->CreateImageView(img_view_cb_info, m_VK_color_buffer_image_views[0]);
        if (result != VK_SUCCESS) {
            SDLOGE("Sample1 color buffer image view failure.");
            return;
        }
        result = m_mgr->CreateImageView(img_view_cb_info, m_VK_color_buffer_image_views[1]);
        if (result != VK_SUCCESS) {
            SDLOGE("Sample1 color buffer image view failure.");
            return;
        }
        ...
        img_view_db_info.image = m_VK_depth_buffers[i];
         ...
         */
        result = m_mgr->CreateImageView(img_view_db_info, m_VK_depth_buffer_image_views[0]);
        if (result != VK_SUCCESS) {
            SDLOGE("Sample1 depth buffer image view failure.");
            return;
        }
        result = m_mgr->CreateImageView(img_view_db_info, m_VK_depth_buffer_image_views[1]);
        if (result != VK_SUCCESS) {
            SDLOGE("Sample1 depth buffer image view failure.");
            return;
        }
